...
namespace TestGeneratorLib
{
    public class CodeAnalyzer
    {
        public NamespaceInfo Analyse(string code)
        {
            ...
            var methodDeclaration = classDeclaration.Members.OfType<MethodDeclarationSyntax>().Where(
                    method => method.Modifiers.Where(modifier => modifier.Kind() == SyntaxKind.PublicKeyword).Any());
